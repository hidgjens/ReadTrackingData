...
int main (int argc, char * argv[])
{
    ...
    std::string     DATA_DIR    = "./msgpk_ex/";
    uint            MATCH_ID    = 918893;
    ...
    uint total_frames   =   ex_match.number_of_frames();
    uint new_total_frames;

    ex_match.remove_dead_frames(true);
    
    new_total_frames    =   ex_match.number_of_frames();

    std::cout << "before: " << total_frames << " after: " << new_total_frames << " change: " << (total_frames - new_total_frames) << std::endl;
